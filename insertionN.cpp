...
void try_insertion(Worker &w, int rid, double &delta, int &optimanl_i, int &optimanl_j) {
	...
	scache.resize(schedule.size()+1, INF), ecache.resize(schedule.size()+1, INF);
	arricache.resize(schedule.size(), INF);
...
void updateDriverArr(Worker& w){
	...
	// 	while ((FunctionTimesegments[k].f->begin()+1) -> t < arr & FunctionTimesegments[k].f->size() >= 50)
	// 	{
	...
	// 	while ((FunctionTimesegments[k].f->begin() + s-1) ->t > d & FunctionTimesegments[k].f->size() >= 50 )
	// 	{
